                ...
                <div class="box-body">
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        @foreach($permissions as $permission)
                            <tr>
                                <td>{{ $permission->id }}</td>
                                <td><span class="label label-info">{{ $permission->slug }}</span></td>
                                <td>{{ $permission->name }}</td>
                                <td>{{ $permission->description }}</td>
                                <td>{{ $permission->created_at }}</td>
                                <td>{{ $permission->updated_at }}</td>
                                <td>
                                    <a href="javascript:;" data-toggle="modal" data-target="#_edit{{ $permission->id }}" class="btn btn-info btn-sm">编辑</a>
                                    <a href="/admin/permission/destroy/{{ $permission->id }}" class="btn btn-warning btn-sm">删除</a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
