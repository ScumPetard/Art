    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($keywords) {
                ...
                $works = Work::where('work_name', 'like', '%' . $keywords . '%')->where('worktype_id', 2)->paginate(16);
            } elseif ($author_id) {
                $where['authorid'] = $author_id;
                $works = Work::where('author_id', $author_id)->where('worktype_id', 2)->paginate(16);
            } elseif ($workdate_id) {
                $where['workdate'] = $workdate_id;
                $workids = WorkAndWorkDate::where('workdate_id', $workdate_id)->lists('work_id');
                $works = Work::where('worktype_id', 2)->whereIn('id', $workids)->paginate(16);
            } else {
                $works = Work::where('worktype_id', 2)->paginate(16);
            }
